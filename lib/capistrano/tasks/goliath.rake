namespace :goliath do
  ...
  task :stop do
    on roles(:app) do
      ...
      goliath_env  = fetch(:goliath_env) || fetch(:stage)
      within current_path do
        execute :kill, "-TERM `ps aux | grep '[r]uby yep_app.rb -e #{goliath_env}' | grep -v grep | cut -c 10-16` > /dev/null 2>&1 || true"
        3.times { print '.'; sleep 1 }
        puts "\n"
        execute :kill, "-9 `ps aux | grep '[r]uby yep_app.rb -e #{goliath_env}' | grep -v grep | cut -c 10-16` > /dev/null 2>&1 || true"
  ...
  # deploy   11603  0.0  1.9 283020 40112 ?        Sl   17:39   0:00 ruby yep_app.rb -e staging -p 9000 -d -l log/staging.log -P tmp/pids/9000.pid
